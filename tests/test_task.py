...
class TestBaseCase(unittest.TestCase):
    ...
    def test_read_file(self):
        filename = "tests/housing_data.json"
        with open(filename) as f:
            expected_result = json.load(f)

        try:
            from task import read_file
        except ImportError:
            raise AssertionError(
                'Проверьте, что объявлена функция `read_file`'
            )

        src_file = "housing_data.csv"
        actual_result = read_file(src_file)

        assert isinstance(actual_result, list), (
            "Убедитесь, что функция `read_file` возвращает нужный тип данных."
        )

        assert len(actual_result) == len(expected_result), (
            f"Убедитесь, что функция `read_file` верно считывает все строки с данными из файла `{src_file}`"
        )

        for row in actual_result:
            assert isinstance(row, dict), (
                "Убедитесь, что функция `read_file` возвращает список словарей."
            )
        ...
        params = [
            ...
            ("house_address", str),
            ("floor_count", int),
            ("heating_house_type", str),
            ("heating_value", float),
            ("area_residential", float),
            ("population", int),
        ]
        for column, expected_type in params:
            with self.subTest(column=column, expected_type=expected_type):
                for row in actual_result:
                    assert column in row, (
                        f"Проверьте, что после обработки файла функцией `read_file` "
                        f"результат содержит значение из столбца `{column}`"
                    )

                    assert isinstance(row[column], expected_type), (
                        f"Проверьте, что после обработки файла функцией `read_file` "
                        f"результат из столбца `{column}` содержит нужный тип данных."
                    )
